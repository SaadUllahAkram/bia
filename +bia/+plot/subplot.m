function h = subplot(rows, cols, plot_num, handle_fig)
% creates subplots with very small margins
% 
% Inputs:
%     rows: 
%     cols:
%     plot_num: 
%     handle_fig: add subplot in this figure
% Outputs:
%     h : handle to subplot axes
% 
    
if nargin < 4
    h = subaxis( rows, cols, plot_num, 'Spacing', 0.01, 'Padding', 0, 'Margin', 0);
else
    h = subaxis( rows, cols, plot_num, 'Spacing', 0.01, 'Padding', 0, 'Margin', 0, 'Parent', handle_fig);
end
    if nargout == 0
        clear h
    end
...
if Args.Holdaxis
    h=axes(Args.Parent, 'position',[xpos1 1-ypos2 xpos2-xpos1 ypos2-ypos1]);
else
